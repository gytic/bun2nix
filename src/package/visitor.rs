...
impl<'de> Visitor<'de> for PackageVisitor {
        ...
        M: MapAccess<'de>,
    {
        ...
        while let Some((name, values)) = map.next_entry::<String, Vec<serde_json::Value>>()? {
            ...
            let meta = serde_json::from_str(&values[2].to_string()).map_err(|e| {
                de::Error::custom(format!("Invalid metadata format: {}", e.to_string()))
            })?;

            let pkg = Package::new(name, npm_identifier, meta);
