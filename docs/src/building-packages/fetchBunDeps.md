...
| `useFakeNode`   | By default, `bun2nix` patches any scripts that use node in your dependencies to use `bun` as it's executable instead. Turning this off will patch them to use `node` instead. This might be useful, if, for example, you need to link to actual node v8 while building a native addon. Defaults to false. |
